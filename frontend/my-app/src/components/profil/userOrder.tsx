...
const UserOrders = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        setLoading(true);
        const response = await apiClient.get('/api/api/order/orders', {
  ...
  const handleProceedToPayment = (orderId, total) => {
      ...
      {orders.map((order) => (
        <div className="order-card" key={order.id}>
          <div className="order-header">
            <div>
              <p><strong>Commande effectuée le :</strong> {new Date(order.createdAt).toLocaleDateString()}</p>
          ...
          <div className="order-items">
            {order.OrderItems.map((item) => (
              <div className="order-item" key={item.id}>
                <img src={item.Article.photo[0]} alt={item.Article.name} className="product-photo" />
                <div>
                  <p><strong>{item.Article.name}</strong></p>
                  <p>Prix : {item.price} €</p>
          ...
          <div className="order-actions">
            {order.status === 'Validé' ? (
              <button className="action-button" onClick={() => handleDownloadInvoice(order.id)}>
