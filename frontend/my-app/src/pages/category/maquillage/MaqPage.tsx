...
const MaquillagePage = () => {
  ...
  const [maquillages, setMaquillages] = useState([]);
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        const response = await apiClient.get('/api/article/category/2'); // Utilise la catégorie maquillage (ID = 3)
  ...
  useEffect(() => {
    maquillages.forEach(async (article: any) => {
      try {
  ...
  return (
    <Container className="mt-4">
      <Row className="custom-row">
        {maquillages.map((article: any, index) => (
