...
export const getCartByUserId = async (req, res) => {
  try {
    ...
    // Rechercher le panier associé à cet utilisateur avec les articles (cartItems)
    const cart = await Cart.findOne({
      where: { user_fk: userId }, // Assure-toi que user_fk est la clé étrangère de l'utilisateur dans le modèle Cart
      include: [
        {
          model: CartItem, // Assure-toi que CartItem est bien associé à Cart
          as: 'cartItems', // Nom de l'association (doit correspondre à votre relation Sequelize)
          include: [
            {
              model: Article, // Inclure les détails des articles dans chaque CartItem
              as: 'article', // Nom de l'association
            },
          ],
        },
      ],
    ...
    // Si le panier est trouvé, renvoyer les détails du panier avec les articles
