...
export const createCart = async (req, res) => {
  ...
  if (existingCart) {
    return res
      .status(200)
      .json({ error: "Un panier existe déjà pour cet utilisateur" });
  }
  ...
  try {
    ...
    return res
      .status(500)
      .json({
        error: "Erreur serveur lors de la création du panier",
...
export const deleteCart = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Erreur serveur lors de la suppression du panier" });
  }
